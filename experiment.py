...
from psynet.js_synth import JSSynth, InstrumentTimbre, Note, ShepardTimbre, Chord, Rest
...
#Ignore (from previous experiment - will remove soon)
SCALES = load_scales("scales.tsv")  # 10 in total
MELODIES = load_melodies("melodies")

#High_level attributes, will probably add low_level (core and basic) later
RATING_ATTRIBUTES = [
    "Wonder",
    "Transcendence",
    "Tenderness",
...
PROBE_TONES = [[i] for i in range(12)]
PROBE_INTERVALS = [
    [i, j]
    for i in range(12)
    for j in range(12)
...
NODES = [
    ...
    #Delete Later (previous experiment)
    for chord in [[60, 64, 67]]
    for probe in (PROBE_TONES + PROBE_INTERVALS)
]
...
class MelodyTrial(StaticTrial):
    ...
    def show_trial(self, experiment, participant):
        context = [
            Chord(self.definition["chord"]),
            Rest(duration=0.5),
        ]
        probe = [
            Note(tone)
            for tone in self.definition["probe"]
        ]
        stimulus = context + probe
    ...
    # def show_trial(self, experiment, participant):
    #     return ModularPage(
    #         "Chord and probe tone",
    #         JSSynth(
    #             (
    #                 "Listen to the following chord. "
...
class Exp(psynet.experiment.Experiment):
    ...
    timeline = Timeline(
        ...
        #     time_estimate=5,
        # ),
        # volume_calibration(mean_pitch=67, sd_pitch=5, timbre=ShepardTimbre()),
        # InfoPage(
        #     """
